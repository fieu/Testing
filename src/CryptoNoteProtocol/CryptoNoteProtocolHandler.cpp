...
bool CryptoNoteProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, CryptoNoteConnectionContext& context, bool is_inital) {
  ...
  if (context.m_state == CryptoNoteConnectionContext::state_synchronizing) {
    ...
    logger(diff >= 0 ? (is_inital ? Logging::INFO : Logging::DEBUGGING) : Logging::TRACE, Logging::BRIGHT_BLUE) << context <<
      "Your Millecoin node is syncing with the network. You are "
...
int CryptoNoteProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, CryptoNoteConnectionContext& context) {
  ...
  logger(DEBUGGING, BRIGHT_BLUE) << "Local blockchain updated, new index = " << m_core.getTopBlockIndex();
...
bool CryptoNoteProtocolHandler::request_missing_objects(CryptoNoteConnectionContext& context, bool check_having_blocks) {
  if (context.m_needed_objects.size()) {
    ...
    context.m_state = CryptoNoteConnectionContext::state_normal;
    logger(Logging::INFO, Logging::BRIGHT_BLUE) << context << "Successfully synchronized with the Millecoin Network.";
    on_connection_synchronized();
  }
  return true;
}
...
bool CryptoNoteProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    logger(Logging::INFO)
      << ENDL ;
      logger(INFO, BRIGHT_BLUE) << "===[ Millecoin Pro-Tip! ]=============================" << ENDL ;
      logger(INFO, WHITE) << " Always exit Millecoind and zedwallet with the \"exit\" command to preserve your chain and wallet data." << ENDL ;
      ...
      logger(INFO, WHITE) << " If you need more assistance, visit the #HELP channel in the Millecoin Discord Chat - http://chat.millecoin.cx" << ENDL ;
      logger(INFO, BRIGHT_BLUE) << "===================================================" << ENDL << ENDL ;

      logger(INFO, BRIGHT_BLUE) <<

      #ifdef _WIN32
              "\n  __  __ ___ _     _     _____ ____ ___ ___ _   _ \n"
              " |  \\/  |_ _| |   | |   | ____/ ___/ _ \\_ _| \\ | |\n"
              " | |\\/| || || |   | |   |  _|| |  | | | | ||  \\| |\n"
              " | |  | || || |___| |___| |__| |__| |_| | || |\\  |\n"
              " |_|  |_|___|_____|_____|_____\\____\\___/___|_| \\_|\n"  << ENDL;
      #else
      "\n                                                                    \n"
        "███╗   ███╗██╗██╗     ██╗     ███████╗ ██████╗ ██████╗ ██╗███╗   ██╗\n"
        "████╗ ████║██║██║     ██║     ██╔════╝██╔════╝██╔═══██╗██║████╗  ██║\n"
...
void CryptoNoteProtocolHandler::updateObservedHeight(uint32_t peerHeight, const CryptoNoteConnectionContext& context) {
  ...
  
  {
    ...
    if (peerHeight > m_blockchainHeight) {
      ...
      logger(Logging::INFO, Logging::BRIGHT_BLUE) << "New Top Block Detected: " << peerHeight; 
    }
  }
